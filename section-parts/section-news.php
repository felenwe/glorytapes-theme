...
<section id="<?php if ( $onepress_news_id != '' ) echo $onepress_news_id; ?>" <?php do_action( 'onepress_section_atts', 'news' ); ?> class="<?php echo esc_attr( apply_filters( 'onepress_section_class', 'section-news section-padding-bottom onepage-section', 'news' ) ); ?>">
	<?php do_action( 'onepress_section_before_inner', 'news' ); ?>
	<div class="container">
		<div class="section-title-area section-title-area-news">
			<?php if ( $desc = get_theme_mod( 'onepress_news_desc' ) ) {
				//echo '<div class="section-desc"><h2>' . wp_kses_post( $desc ) . '</h2></div>';
				echo '<img style="margin-bottom:40px;" src="' . wp_kses_post( $desc ) . '"/>';
            } ?>
